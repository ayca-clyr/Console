...
namespace Odev1
{
    class Program
    {
        ...
        int sayac = 0;
        int yarıUzunluk = dizi1.Length / 2;
        
            while (sayac < yarıUzunluk)   // Yarısını almazsak tekrar bunu başa döndürmüş olur.
        {
            int kova = dizi1[sayac];
            dizi1[sayac] = dizi1[dizi1.Length-1-sayac];
            dizi1[dizi1.Length-1-sayac] = kova;          
            sayac++;
        }
        sayac = 0;
            while(sayac < dizi1.Length)
